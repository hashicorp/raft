...
func firstIndex(a *raft.AppendEntriesRequest) raft.Index {
	if len(a.Entries) == 0 {
		return 0
	}
...
func lastIndex(a *raft.AppendEntriesRequest) raft.Index {
	if len(a.Entries) == 0 {
		return 0
	}
...
func (t *transport) AppendEntriesPipeline(target raft.ServerAddress) (raft.AppendPipeline2, error) {
	p := &pipeline{
		t:      t,
		target: target,

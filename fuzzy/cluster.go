...
func newRaftCluster(t *testing.T, logWriter io.Writer, namePrefix string, n uint, transportHooks TransportHooks) *cluster {
	...
	for _, i := range names {
		r, err := newRaftNode(raft.NewRaftLoggerWithWriter(logWriter, logxi.LevelDebug), transports, transportHooks, names, i)
		if err != nil {
...
func (c *cluster) CreateAndAddNode(t *testing.T, logWriter io.Writer, namePrefix string, nodeNum uint) error {
	...
	rn, err := newRaftNode(raft.NewRaftLoggerWithWriter(logWriter, logxi.LevelDebug), c.transports, c.hooks, nil, name)
	if err != nil {
...
func (c *cluster) Leader(timeout time.Duration) *raftNode {
	...
	for true {
		for _, n := range c.nodes {
			sf := n.raft.Stats()
			if sf.Error() == nil && sf.Stats().State == raft.Leader {
				return n
			}
		}
...
func (c *cluster) WaitTilUptoDate(t *testing.T, maxWait time.Duration) {
	...
	start := time.Now()
	statsFutures := make([]raft.StatsFuture, len(c.nodes))
	for true {
		for i := 0; i < len(c.nodes); i++ {
			statsFutures[i] = c.nodes[i].Stats()
		}
		allAtIdx := true
		for i, f := range statsFutures {
			if err := f.Error(); err != nil {
				t.Fatalf("Error getting stats for node %d: %v", i, err)
			}
			nodeAppliedIdx := f.Stats().AppliedIndex - raft.Index(f.Stats().FSMPending)
...
func (c *cluster) appliedIndexes() map[string]raft.Index {
	r := make(map[string]raft.Index, len(c.nodes))
	f := make([]raft.StatsFuture, len(c.nodes))
	for i, n := range c.nodes {
		f[i] = n.Stats()
	}
	for i, sf := range f {
		if err := sf.Error(); err != nil {
			c.log.Logf("Failed to get stats for node %v: %v", c.nodes[i].name, err)
			continue
		}
		r[c.nodes[i].name] = sf.Stats().AppliedIndex
	}
	return r
}

func (c *cluster) ApplyN(t *testing.T, leaderTimeout time.Duration, s *applySource, n uint) raft.Index {
...
func (c *cluster) VerifyLog(t *testing.T, applyCount raft.Index) {

...
func TestSQLite_StableStore(t *testing.T) {
	var l interface{} = &SQLiteStore{}
	_, ok := l.(StableStore)
	if !ok {
		t.Fatalf("SQLite is not StableStore")
	}
}
...
func TestSQLite_SetGetUint64(t *testing.T) {
	// Create a test dir
	dir, err := ioutil.TempDir("", "raft")
	if err != nil {
		t.Fatalf("err: %v ", err)
...
func TestSQLite_LogStore(t *testing.T) {
	var l interface{} = &SQLiteStore{}
	_, ok := l.(LogStore)
	if !ok {
		t.Fatalf("SQLiteStore is not a LogStore")
	}
}

func TestSQLite_Logs(t *testing.T) {
	// Create a test dir
	dir, err := ioutil.TempDir("", "raft")
	if err != nil {
		t.Fatalf("err: %v ", err)

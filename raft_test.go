...
func TestRaft_RecoverCluster(t *testing.T) {
	...
	for applies := 0; applies < 20; applies++ {
		t.Run(fmt.Sprintf("applies=%v", applies), func(t *testing.T) {
...
func TestRaft_LeaderLeaseExpire(t *testing.T) {
	...
	// Watch the leaderCh
	stepDownTimeoutCh := time.After(conf.LeaderLeaseTimeout * 2)
...
WAIT_FOR_STEPDOWN:
	for {
		...
		case v := <-leader.LeaderCh():
			if !v {
				break WAIT_FOR_STEPDOWN
			}
		case <-stepDownTimeoutCh:

	...
	for {
		...
		case <-heartbeatTimer:
			...
			if r.configurations.latestIndex == 0 {
				...
				r.logger.Warn(fmt.Sprintf("Heartbeat timeout from leader reached, starting election", "leader", hclog.Fmt("%q", lastLeader))
...
func (r *Raft) runCandidate() {
	r.logger.Info("entering Candidate state in next term %v", "candidate", r, "term", r.getCurrentTerm()+1, "currentTerm", r.getCurrentTerm()))
	...
	for r.getState() == Candidate {
		...
		case vote := <-voteCh:
			...
			if grantedVotes >= votesNeeded {
				r.logger.Info("Election won.", "talley", grantedVotes)
				r.setState(Leader)
...
func (r *Raft) startStopReplication() {
	...
	for _, server := range r.configurations.latest.Servers {
		...
		if _, ok := r.leaderState.replState[server.ID]; !ok {
			r.logger.Info(fmt.Sprintf("Added peer, starting replication", "peer", server.ID))
	...
	for serverID, repl := range r.leaderState.replState {
		...
		r.logger.Info("Removed peer, stopping replication after last index", "peer", serverID, "lastIndex"lastIdx)
		repl.stopCh <- lastIdx
		close(repl.stopCh)
	...
	for peer, f := range r.leaderState.replState {
		...
		if diff <= r.conf.LeaderLeaseTimeout {
			...
			if diff <= 3*r.conf.LeaderLeaseTimeout {
				r.logger.Warn(fmt.Sprintf("Failed to contact peer in %v", diff), "peer", peer)
			} else {
				r.logger.Debug(fmt.Sprintf("Failed to contact peer in %v", diff), "peer", peer)
			}
		}
...
func (r *Raft) appendConfigurationEntry(future *configurationChangeFuture) {
	...
	r.logger.Info(fmt.Sprintf("Updating configuration with new entry",
		"entry", future.req.command,
		"serverID", future.req.serverID,
...
func (r *Raft) processLogs(index uint64, future *logFuture) {
	...
	if index <= lastApplied {
		r.logger.Warn("Skipping application of old log", "index", hclog.Fmt("%d", index))
		return
	}
	...
	for idx := r.getLastApplied() + 1; idx <= index; idx++ {
		...
		if future != nil && future.log.Index == idx {
			...
			if err := r.logs.GetLog(idx, l); err != nil {
			  r.logger.Error("Failed to get log" "index", hclog.Fmt("%d", idx), "error", err)
				panic(err)
			}
			r.processLog(l, nil)
		}
...
func (r *Raft) processRPC(rpc RPC) {
	...
	default:
		r.logger.Error("Got unexpected RPC command", "command", hclog.Fmt("%#v", rpc.Command))
...
func (r *Raft) processHeartbeat(rpc RPC) {
	...
	case *AppendEntriesRequest:
		r.appendEntries(rpc, cmd)
	default:
		r.logger.Error("Expected heartbeat, got command", "command", hclog.Fmt("%#v", rpc.Command))
		rpc.Respond(nil, fmt.Errorf("unexpected command"))
	}
}
	...
	if a.PrevLogEntry > 0 {
		...
		if a.PrevLogEntry == lastIdx {
			...
			if err := r.logs.GetLog(a.PrevLogEntry, &prevLog); err != nil {
				r.logger.Warn("Failed to get previous log", "previous", hclog.Fmt("%d", a.PrevLogEntry), "error", err, "lastIndex", hclog.Fmt(lastIdx))
		...
		if a.PrevLogTerm != prevLogTerm {
			r.logger.Warn("Previous log term mis-match", "ours", hclog.Fmt("%d", prevLogTerm), "remote", hclog.Fmt("%d", a.PrevLogTerm))
			resp.NoRetryBackoff = true
			return
		}
	}
	...
	if len(a.Entries) > 0 {
		...
		for i, entry := range a.Entries {
			...
			if err := r.logs.GetLog(entry.Index, &storeEntry); err != nil {
				r.logger.Warn("Failed to get log entry", "entry" hclog.Fmt("%d", entry.Index), "error", err)
				return
			}
			if entry.Term != storeEntry.Term {
				r.logger.Warn("Clearing log suffix", "from", hclog.Fmt("%d", entry.Index), "to", hclog.Fmt("%d", lastLogIdx))
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if leader := r.Leader(); leader != "" && leader != candidate {
	  r.logger.Warn("Rejecting vote request from candidate since we have a leader", "candidate", candidate, "leader", leader)
		return
	}
	...
	if lastVoteTerm == req.Term && lastVoteCandBytes != nil {
		r.logger.Info("Duplicate RequestVote for same term", "term", hclog.Fmt("%d", req.Term))
		if bytes.Compare(lastVoteCandBytes, req.Candidate) == 0 {
			r.logger.Warn("Duplicate RequestVote from candidate", "candidate", hclog.Fmt("%s", req.Candidate))
			resp.Granted = true
		}
		return
	}
	...
	if lastTerm > req.LastLogTerm {
	  r.logger.Warn(fmt.Sprintf("Rejecting vote request from candidate since our last term is greater", "candidate", candidate, "ourTerm", hclog.Fmt("%d", lastTerm), "theirTerm", hclog.Fmt("%d", req.LastLogTerm))
		return
	}

	if lastTerm == req.LastLogTerm && lastIdx > req.LastLogIndex {
		r.logger.Warn("Rejecting vote request from candidate since our last index is greater", "candidate", hclog.Fmt("%d", candidate), "ourIndex", hclog.Fmt("%d", lastIdx), "theirIndex", hclog.Fmt("%d", req.LastLogIndex))
		return
	}
...
func (r *Raft) installSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	// Ignore an older term
	if req.Term < r.getCurrentTerm() {
		r.logger.Info("Ignoring installSnapshot request with older term", "ourTerm", hclog.Fmt("%d", r.getCurrentTerm()), "theirTerm", hclog.Fmt("%d", req.Term))
		return
	}
...
func (r *Raft) electSelf() <-chan *voteResult {
	...
	askPeer := func(peer Server) {
		r.goFunc(func() {
			...
			if err != nil {
				r.logger.Error("Failed to make RequestVote RPC to peer", "peer", peer, "error", err)
				resp.Term = req.Term
				resp.Granted = false
			}
			respCh <- resp

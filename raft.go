...
func (r *Raft) runFollower() {
	didWarn := false
	r.logger.Logf(LogInfo, "raft: %v entering Follower state (Leader: %q)", r, r.Leader())
	...
	for {
		...
		case <-heartbeatTimer:
			...
			if r.configurations.latestIndex == 0 {
				if !didWarn {
					r.logger.Logf(LogWarn, "raft: no known peers, aborting election")
					didWarn = true
				}
			} else if r.configurations.latestIndex == r.configurations.committedIndex &&
				!hasVote(r.configurations.latest, r.localID) {
				if !didWarn {
					r.logger.Logf(LogWarn, "raft: not part of stable configuration, aborting election")
					didWarn = true
				}
			} else {
				r.logger.Logf(LogWarn, ` raft: Heartbeat timeout from %q reached in term %d (last contact %v), starting election`,
...
func (r *Raft) runCandidate() {
	r.logger.Logf(LogInfo, "raft: %v entering Candidate state in term %v",
	...
	if hasVote(r.configurations.latest, r.localID) {
		...
		if err != nil {
			r.logger.Logf(LogError, "raft: Failed to persist vote : %v", err)
			return // TODO: panic?
		}
	}
	...
	for r.getState() == Candidate {
		...
		case <-electionTimer:
			...
			r.logger.Logf(LogWarn, "raft: Election timeout reached, restarting election")
			return
...
func (r *Raft) runLeader() {
	r.logger.Logf(LogInfo, "raft: %v entering Leader state", r)
...
func (r *Raft) updatePeers() {
	...
	for _, server := range r.configurations.latest.Servers {
		...
		if _, ok := r.peers[server.ID]; !ok {
			r.logger.Logf(LogInfo, "raft: Added peer %v, starting replication", server.ID)
	...
	for serverID, peer := range r.peers {
		...
		if !ok {
			r.logger.Logf(LogInfo, "raft: Removed peer %v, stopping communication", serverID)
...
func (r *Raft) shutdownPeers() {
	...
	for serverID, peer := range r.peers {
		r.logger.Logf(LogInfo, "raft: Shutting down communication with peer %v", serverID)
		peer.controlCh <- control
		delete(r.peers, serverID)
		r.logger.Logf(LogInfo, "raft: Shut down communication with peer %v", serverID)
	}
}
...
func (r *Raft) updateCommitIndex(oldCommitIndex, commitIndex Index) {
	...
	if stepDown {
		if r.conf.ShutdownOnRemove {
			r.logger.Logf(LogInfo, "raft: Removed ourself, shutting down")
			r.Shutdown()
		} else {
			r.logger.Logf(LogInfo, "raft: Removed ourself, transitioning to follower")
			r.stepDown()
		}
	}
...
func (r *Raft) verified(count uint64) {
	...
	if i > 0 {
		r.logger.Logf(LogInfo, "raft: Verified leadership through counter %v", count)
...
func (r *Raft) computeCandidateProgress() {
	...
	for peerID, peer := range r.peers {
		...
		case peer.progress.term > r.getCurrentTerm():
			r.logger.Logf(LogDebug, "raft: Newer term discovered, fallback to follower")
	...
	if quorumGeq(votes) == 1 {
		r.logger.Logf(LogInfo, "raft: Election won. Tally: %d", sum(votes))
		r.setState(Leader)
...
func (r *Raft) verifyLeader(futures []*verifyFuture) {
	...
	r.logger.Logf(LogInfo, "raft: Verifying leadership for %v requests with counter %v",
		len(futures), counter)
...
func (r *Raft) checkLeaderLease() {
	...
	if r.conf.LeaderLeaseTimeout < diff {
		r.logger.Logf(LogWarn, "raft: Failed to contact quorum of nodes, stepping down")
		r.stepDown()
...
func (r *Raft) appendConfigurationEntry(future *configurationChangeFuture) {
	...
	r.logger.Logf(LogInfo, "raft: Updating configuration with %s (%v, %v) to %+v",
...
func (r *Raft) dispatchLogs(applyLogs []*logFuture) {
	...
	if err := r.logs.StoreLogs(logs); err != nil {
		r.logger.Logf(LogError, "raft: Failed to commit logs: %v", err)
...
func (r *Raft) processLogs(index Index, future *logFuture) {
	...
	if index <= lastApplied {
		r.logger.Logf(LogWarn, "raft: Skipping application of old log: %d", index)
		return
	}
	...
	for idx := r.getLastApplied() + 1; idx <= index; idx++ {
		...
		if future != nil && future.log.Index == idx {
			...
			if err := r.logs.GetLog(idx, l); err != nil {
				r.logger.Logf(LogError, "raft: Failed to get log at %d: %v", idx, err)
				panic(err)
			}
			r.processLog(l, nil)
		}
...
func (r *Raft) processRPC(rpc RPC) {
	...
	default:
		r.logger.Logf(LogError, "raft: Got unexpected command: %#v", rpc.Command)
...
func (r *Raft) processHeartbeat(rpc RPC) {
	...
	case *AppendEntriesRequest:
		r.appendEntries(rpc, cmd)
	default:
		r.logger.Logf(LogError, "raft: Expected heartbeat, got command: %#v", rpc.Command)
		rpc.Respond(nil, fmt.Errorf("unexpected command"))
	}
}
	...
	if a.PrevLogEntry > 0 {
		...
		if a.PrevLogEntry == lastIdx {
			...
			if err := r.logs.GetLog(a.PrevLogEntry, &prevLog); err != nil {
				r.logger.Logf(LogWarn, "raft: Failed to get previous log entry %d: %v (last: %d)",
		...
		if a.PrevLogTerm != prevLogTerm {
			r.logger.Logf(LogWarn, "raft: Previous log term mis-match: ours: %d remote: %d",
	...
	if len(a.Entries) > 0 {
		...
		for i, entry := range a.Entries {
			...
			if err := r.logs.GetLog(entry.Index, &storeEntry); err != nil {
				r.logger.Logf(LogWarn, "raft: Failed to get log entry %d: %v",
					entry.Index, err)
				return
			}
			if entry.Term != storeEntry.Term {
				r.logger.Logf(LogWarn, "raft: Clearing log suffix from %d to %d", entry.Index, lastLogIdx)
				if err := r.logs.DeleteRange(entry.Index, lastLogIdx); err != nil {
					r.logger.Logf(LogError, "raft: Failed to clear log suffix: %v", err)
					return
				}
		...
		if n := len(newEntries); n > 0 {
			...
			if err := r.logs.StoreLogs(newEntries); err != nil {
				r.logger.Logf(LogError, "raft: Failed to append to logs: %v", err)
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if leader := r.Leader(); leader != "" && leader != candidate {
		r.logger.Logf(LogWarn, "raft: Rejecting vote request from %v since we have a leader: %v",
			candidate, leader)
		return
	}
	...
	if err != nil && err.Error() != "not found" {
		r.logger.Logf(LogError, "raft: Failed to get last vote term: %v", err)
		return
	}
	lastVoteCandBytes, err := r.stable.Get(keyLastVoteCand)
	if err != nil && err.Error() != "not found" {
		r.logger.Logf(LogError, "raft: Failed to get last vote candidate: %v", err)
		return
	}
	...
	if lastVoteTerm == req.Term {
		if lastVoteCandBytes != nil && bytes.Compare(lastVoteCandBytes, req.Candidate) == 0 {
			r.logger.Logf(LogWarn, "raft: Duplicate RequestVote in term %d from candidate %s", req.Term, req.Candidate)
			resp.Granted = true
		} else {
			r.logger.Logf(LogInfo, "raft: Already voted for %s, cannot grant vote in term %d to candidate %s",
	...
	if lastTerm > req.LastLogTerm {
		r.logger.Logf(LogWarn, "raft: Rejecting vote request from %v since our last term is greater (%d, %d)",
			candidate, lastTerm, req.LastLogTerm)
		return
	}

	if lastTerm == req.LastLogTerm && lastIdx > req.LastLogIndex {
		r.logger.Logf(LogWarn, "raft: Rejecting vote request from %v since our last index is greater (%d, %d)",
...
func (r *Raft) installSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	if err != nil {
		r.logger.Logf(LogError, "raft: Failed to create snapshot to install: %v", err)
	...
	if err != nil {
		sink.Cancel()
		r.logger.Logf(LogError, "raft: Failed to copy snapshot: %v", err)
		rpcErr = err
		return
	}
	...
	if n != req.Size {
		sink.Cancel()
		r.logger.Logf(LogError, "raft: Failed to receive whole snapshot: %d / %d", n, req.Size)
	...
	if err := sink.Close(); err != nil {
		r.logger.Logf(LogError, "raft: Failed to finalize snapshot: %v", err)
		rpcErr = err
		return
	}
	r.logger.Logf(LogInfo, "raft: Copied %d bytes to local snapshot", n)
	...
	if err := future.Error(); err != nil {
		r.logger.Logf(LogError, "raft: Failed to restore snapshot: %v", err)
		rpcErr = err
		return
	}
	...
	if err := r.compactLogs(req.LastLogIndex); err != nil {
		r.logger.Logf(LogError, "raft: Failed to compact logs: %v", err)
	}

	r.logger.Logf(LogInfo, "raft: Installed remote snapshot")
	resp.Success = true
	return
}

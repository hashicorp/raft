	...
	for {
		...
		case <-heartbeatTimer:
			...
			if r.configurations.latestIndex == 0 {
				...
				r.logger.Warn("Heartbeat timeout from %q reached, starting election", lastLeader)
...
func (r *Raft) runCandidate() {
	...
	r.logger.Debug("Votes needed: %d", votesNeeded)
	...
	for r.getState() == Candidate {
		...
		case vote := <-voteCh:
			...
			if vote.Granted {
				grantedVotes++
				r.logger.Debug("Vote granted from %s in term %v. Tally: %d",
					vote.voterID, vote.Term, grantedVotes)
			}

			// Check if we've become the leader
			if grantedVotes >= votesNeeded {
				r.logger.Info("Election won. Tally: %d", grantedVotes)
				r.setState(Leader)
...
func (r *Raft) startStopReplication() {
	...
	for serverID, repl := range r.leaderState.replState {
		...
		r.logger.Info("Removed peer %v, stopping replication after %v", serverID, lastIdx)
		repl.stopCh <- lastIdx
		close(repl.stopCh)
	...
	for peer, f := range r.leaderState.replState {
		...
		if diff <= r.conf.LeaderLeaseTimeout {
			...
			if diff <= 3*r.conf.LeaderLeaseTimeout {
				r.logger.Warn("Failed to contact %v in %v", peer, diff)
			} else {
				r.logger.Debug("Failed to contact %v in %v", peer, diff)
			}
		}
...
func (r *Raft) restoreUserSnapshot(meta *SnapshotMeta, reader io.Reader) error {
	...
	r.logger.Info("Restored user snapshot (index %d)", lastIndex)
	return nil
}
	...
	if a.PrevLogEntry > 0 {
		...
		if a.PrevLogEntry == lastIdx {
			...
			if err := r.logs.GetLog(a.PrevLogEntry, &prevLog); err != nil {
				r.logger.Warn("Failed to get previous log: %d %v (last: %d)",
...
func (r *Raft) requestVote(rpc RPC, req *RequestVoteRequest) {
	...
	if lastVoteTerm == req.Term && lastVoteCandBytes != nil {
		r.logger.Info("Duplicate RequestVote for same term: %d", req.Term)
		if bytes.Compare(lastVoteCandBytes, req.Candidate) == 0 {
			r.logger.Warn("Duplicate RequestVote from candidate: %s", req.Candidate)
			resp.Granted = true
		}
		return
	}
...
func (r *Raft) installSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	// Ignore an older term
	if req.Term < r.getCurrentTerm() {
		r.logger.Info("Ignoring installSnapshot request with older term of %d vs currentTerm %d", req.Term, r.getCurrentTerm())
		return
	}
...
func (r *Raft) electSelf() <-chan *voteResult {
	...
	askPeer := func(peer Server) {
		r.goFunc(func() {
			...
			if err != nil {
				r.logger.Error("Failed to make RequestVote RPC to %v: %v", peer, err)
				resp.Term = req.Term
				resp.Granted = false
			}
			respCh <- resp

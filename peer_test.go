	...
	go func() {
		for len(replies) > 0 {
			...
			case rpc := <-tp.peerTrans.Consumer():
				...
				if ok {
					tp.peer.shared.logger.Logf(LogInfo, "Peer Test: Replying to %T with error", rpc.Command)
					rpc.Respond(nil, errReply)
				} else {
					tp.peer.shared.logger.Logf(LogInfo, "Peer Test: Replying to %T", rpc.Command)

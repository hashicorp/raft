...
START:
	...
	if err := r.trans.AppendEntries(s.peer.ID, s.peer.Address, &req, &resp); err != nil {
		r.logger.Error("Failed to AppendEntries to peer", "peer", s.peer, "error", err)
		s.failures++
		return
	}
	...
	if resp.Success {
		...
		r.logger.Warn("AppendEntries to peer rejected, sending older logs", "peer", s.peer, "nextIndex", hclog.Fmt("%d", s.nextIndex))
	}
...
func (r *Raft) heartbeat(s *followerReplication, stopCh chan struct{}) {
	...
	for {
		...
		start := time.Now()
		if err := r.trans.AppendEntries(s.peer.ID, s.peer.Address, &req, &resp); err != nil {
			r.logger.Error("Failed to heartbeat to peer", "peer", s.peer, "address", s.peer.Address, "error", err)
			failures++
			select {
...
func (r *Raft) pipelineSend(s *followerReplication, p AppendPipeline, nextIdx *uint64, lastIndex uint64) (shouldStop bool) {
	...
	if _, err := p.AppendEntries(req, new(AppendEntriesResponse)); err != nil {
		r.logger.Error("Failed to pipeline AppendEntries to peer", "peer", s.peer, "error", err)
		return true
	}
...
func (r *Raft) setPreviousLog(req *AppendEntriesRequest, nextIndex uint64) error {
	...
	if nextIndex == 1 {
		...
		if err := r.logs.GetLog(nextIndex-1, &l); err != nil {
			r.logger.Error("Failed to get log", "index", hclog.Fmt("%d", nextIndex-1), "error": err)
			return err
		}
...
func (r *Raft) setNewLogs(req *AppendEntriesRequest, nextIndex, lastIndex uint64) error {
	...
	for i := nextIndex; i <= maxIndex; i++ {
		...
		if err := r.logs.GetLog(i, oldLog); err != nil {
			r.logger.Error("Failed to get log", "index", hclog.Fmt("%d", i), "error", err)
			return err
		}

...
PIPELINE:
	...
	if err := r.pipelineReplicate(s); err != nil {
		if err != ErrPipelineReplicationNotSupported {
			r.logger.Error("Failed to start pipeline replication to %s: %s", s.peer, err)
		}
	}
	goto RPC
}
...
START:
	...
	if err := r.trans.AppendEntries(s.peer.ID, s.peer.Address, &req, &resp); err != nil {
		r.logger.Error("Failed to AppendEntries to %v: %v", s.peer, err)
		s.failures++
		return
	}
	...
	if resp.Success {
		...
		r.logger.Warn("AppendEntries to %v rejected, sending older logs (next: %d)", s.peer, s.nextIndex)
	}
...
SEND_SNAP:
	if stop, err := r.sendLatestSnapshot(s); stop {
		...
		r.logger.Error("Failed to send snapshot to %v: %v", s.peer, err)
		return
	}
...
func (r *Raft) sendLatestSnapshot(s *followerReplication) (bool, error) {
	...
	if err != nil {
		r.logger.Error("Failed to open snapshot %v: %v", snapID, err)
		return false, err
	}
	...
	if err := r.trans.InstallSnapshot(s.peer.ID, s.peer.Address, &req, &resp, snapshot); err != nil {
		r.logger.Error("Failed to install snapshot %v: %v", snapID, err)
		s.failures++
		return false, err
	}
	...
	if resp.Success {
		...
		s.failures++
		r.logger.Warn("InstallSnapshot to %v rejected", s.peer)
	}
	return false, nil
}
...
func (r *Raft) heartbeat(s *followerReplication, stopCh chan struct{}) {
	...
	for {
		...
		start := time.Now()
		if err := r.trans.AppendEntries(s.peer.ID, s.peer.Address, &req, &resp); err != nil {
			r.logger.Error("Failed to heartbeat to %v: %v", s.peer.Address, err)
			failures++
			select {
...
func (r *Raft) pipelineReplicate(s *followerReplication) error {
	...
	r.logger.Info("pipelining replication to peer %v", s.peer)
	defer r.logger.Info("aborting pipeline replication to peer %v", s.peer)
...
func (r *Raft) pipelineSend(s *followerReplication, p AppendPipeline, nextIdx *uint64, lastIndex uint64) (shouldStop bool) {
	...
	if _, err := p.AppendEntries(req, new(AppendEntriesResponse)); err != nil {
		r.logger.Error("Failed to pipeline AppendEntries to %v: %v", s.peer, err)
		return true
	}
...
func (r *Raft) setPreviousLog(req *AppendEntriesRequest, nextIndex uint64) error {
	...
	if nextIndex == 1 {
		...
		if err := r.logs.GetLog(nextIndex-1, &l); err != nil {
			r.logger.Error("Failed to get log at index %d: %v",
				nextIndex-1, err)
			return err
		}
...
func (r *Raft) setNewLogs(req *AppendEntriesRequest, nextIndex, lastIndex uint64) error {
	...
	for i := nextIndex; i <= maxIndex; i++ {
		...
		if err := r.logs.GetLog(i, oldLog); err != nil {
			r.logger.Error("Failed to get log at index %d: %v", i, err)
			return err
		}
...
func (r *Raft) handleStaleTerm(s *followerReplication) {
	r.logger.Error("peer %v has newer term, stopping replication", s.peer)

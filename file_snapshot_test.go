...
func TestFileSS_CreateSnapshotMissingParentDir(t *testing.T) {
	...
	_, err = snap.Create(SnapshotVersionMax, 10, 3, Membership{}, 0, trans)
	if err != nil {
...
func TestFileSS_CreateSnapshot(t *testing.T) {
	...
	_, trans := NewInmemTransport(NewInmemAddr())
	sink, err := snap.Create(SnapshotVersionMax, 10, 3, membership, 2, trans)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestFileSS_CancelSnapshot(t *testing.T) {
	...
	// Create a new sink
	_, trans := NewInmemTransport(NewInmemAddr())
	sink, err := snap.Create(SnapshotVersionMax, 10, 3, Membership{}, 0, trans)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestFileSS_Retention(t *testing.T) {
	...
	_, trans := NewInmemTransport(NewInmemAddr())
	for i := 10; i < 15; i++ {
		sink, err := snap.Create(SnapshotVersionMax, Index(i), 3, Membership{}, 0, trans)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
		err = sink.Close()
...
func TestFileSS_Ordering(t *testing.T) {
	...
	// Create a new sink
	_, trans := NewInmemTransport(NewInmemAddr())
	sink, err := snap.Create(SnapshotVersionMax, 130350, 5, Membership{}, 0, trans)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	err = sink.Close()
	...
	sink, err = snap.Create(SnapshotVersionMax, 204917, 36, Membership{}, 0, trans)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	err = sink.Close()
